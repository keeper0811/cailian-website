...
check(/--header-height:\s*60px/.test(css), 'shared navigation height is 60px');
check(/--hero-height:\s*550px/.test(css), 'shared desktop hero height is 550px');
check(/\.site-header\s*\{[^}]*height:\s*var\(--header-height\)[^}]*min-height:\s*var\(--header-height\)/.test(css), 'site header uses the shared 60px height');
check(/\.home-hero\s*\{[^}]*height:\s*var\(--hero-height\)[^}]*min-height:\s*var\(--hero-height\)/.test(css), 'desktop hero uses the shared 550px height');
check(/\.hero-visual\s*\{[^}]*min-height:\s*var\(--hero-height\)/.test(css), 'desktop hero image fills the 550px hero');
check(/\.hero-copy\s*\{[^}]*min-height:\s*0/.test(css), 'translated hero copy can shrink within the fixed desktop height');
check(/@media \(max-width: 767px\)[\s\S]*?\.home-hero\s*\{[^}]*height:\s*auto[^}]*min-height:\s*auto/.test(css), 'mobile hero keeps an adaptive height for translated content');
check(/\.mobile-menu\s*\{[^}]*top:\s*var\(--header-height\)/.test(css), 'mobile menu begins below the 60px header');
