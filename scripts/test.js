...
check(/--header-height:\s*65px/.test(css), 'shared navigation height is 65px');
check(/--hero-height:\s*450px/.test(css), 'shared desktop hero height is 450px');
check(/\.site-header\s*\{[^}]*height:\s*var\(--header-height\)[^}]*min-height:\s*var\(--header-height\)/.test(css), 'site header uses the shared 65px height');
check(/\.home-hero\s*\{[^}]*height:\s*var\(--hero-height\)[^}]*min-height:\s*var\(--hero-height\)/.test(css), 'desktop hero uses the shared 450px height');
check(/\.hero-visual\s*\{[^}]*min-height:\s*var\(--hero-height\)/.test(css), 'desktop hero image fills the 450px hero');
...
check(/\.mobile-menu\s*\{[^}]*top:\s*var\(--header-height\)/.test(css), 'mobile menu begins below the 65px header');
